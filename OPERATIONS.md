...
## 21. LFG Connect: full-visibility "customer" account for Apple (15 Sept 2026)

Task feedback (Mahin, verbatim, with two screenshots of the Partners admin page listing Apple/IandS/MMDI as partners): "We need a partner account infact customer accoutn where he can see all site at once. Apple wanted to see them. so give prevelege to see whoever logged in as Apple."

**Investigated before writing anything.** LFG Connect already had half of this solved: `supabase-lfg-partner-view-all-sites-migration.sql` (2 Sept 2026, pre-existing) already widened the Site Master's own SELECT policy so any signed-in partner — Apple included — can flip the existing "My Sites / All Sites" toggle and see every site's master fields (status, location, material, size, ASM contact). That migration's own comment calls the gap "expected, not a bug": every CHILD table (survey, documents, shipments, installation, installation photos, issues, status history) stayed scoped to the viewer's own `partner_id`, so a real installation partner browsing another installer's site from the All-Sites list still couldn't see that installer's operational detail. That's the right behavior for two competing installers (IandS shouldn't see MMDI's install photos or vice versa) but Apple isn't an installer — Apple is the client, so seeing full detail across every partner's work is exactly what was asked for.

**Design: a targeted flag, not a blanket policy change.** Followed the exact precedent already set by `supabase-lfg-full-lifecycle-partner-migration.sql`'s `is_full_lifecycle_partner` flag: new `lfg_partners.can_view_all_site_details` boolean (default `false` — every existing and future partner stays exactly as scoped as before unless explicitly flagged), a matching `lfg_partner_can_view_all_site_details()` SQL helper, and the flag is OR'd into the SELECT policy of the 11 child tables the Sept 2 migration deliberately left untouched, plus `lfg_stores` (same "master data, safe to see" reasoning, also untouched by that migration). Every change is SELECT-only — INSERT/UPDATE/DELETE policies on all 11 tables are completely untouched, so this is view-only regardless of the flag (Apple has no sites of its own assigned to its `partner_id`, so it couldn't write to any of these tables before this change either).

**Deliberately not touched:** `lfg_site_financials` and `lfg_installation_costs` — per `lfg-auth.ts`'s own "SECURITY REMINDER" comment, these stay admin/editor-only with zero partner-facing RLS grant, full stop. A customer-visibility flag must never reach MMDI's internal cost/margin data. Also didn't touch `lfg_sites_select` itself — already correctly widened by the Sept 2 migration, no need to duplicate it.

**Migration ends with `update public.lfg_partners set can_view_all_site_details = true where name = 'Apple';`** — targets only the partner row named exactly "Apple." To extend this to another genuine customer-visibility account later, rerun that one line with the new partner's name.

No app code changes — this is RLS-only. Apple's existing login already reaches every one of these tables through the same queries every partner uses on `/lfg/(app)/sites/[siteId]`; the widened policies are what changes what comes back, not the pages themselves.

Verified: `supabase-lfg-customer-full-visibility-migration.sql` parses cleanly under `pglast.parse_sql`. No app code touched, so no `tsc`/`eslint` run needed for this change.

Requires a manual SQL step — run `supabase-lfg-customer-full-visibility-migration.sql` in the Supabase SQL Editor. Idempotent and safe to re-run.

## 22. Customer Portal: "Creative Approval" tab — consolidated design-proof queue (16 Sept 2026)

Task feedback (Mahin, verbatim): "Lets introduce a fresh tab for creative approval and place the designs there and back and forth revisions." Came right after walking through how the existing per-order design-proof/approval flow works (upload → approve/request-revision → repeat, all logged in `portal_order_approvals`) — that flow already existed and was already live on `main` (confirmed: it predates this session, shipped in the original `0968fbf` Customer Portal commit), but it only ever lived on each order's own detail page, one order at a time.

**What this adds — a cross-order view, not a new flow.** Two clarifying questions settled the scope: this is additive on both sides (customer-facing and staff-facing), and it sits alongside the existing per-order proof card on `/portal/orders/[orderId]` rather than replacing it — that page (`OrderDetailClient.tsx`) is untouched.

- **Customer side** — new nav tab "Creative Approval" (`portal/(app)/creative-approval/page.tsx` + `CreativeApprovalClient.tsx`), between Orders and Account in `PortalTopBar.tsx`. Lists every order that has had at least one proof uploaded (`current_revision_number > 0`), split into "Needs your review" (status `proof_uploaded`/`revision_requested`, with the same Approve / Request revision UI as the order page, calling the exact same `/approve` and `/request-revision` endpoints) and "Previously reviewed" (a compact history list linking into each order's full page). The home page's "Orders needing your review" card now points here instead of at `/orders`, since this tab is the more direct answer to that exact card's own promise.
- **Staff side** — new "Creative Approval" tab in Tools → Customer Portal (`CreativeApprovalTab.tsx`), next to Orders. Two lists: "Needs a proof from you" (orders `submitted` or `revision_requested` — the latter shows the customer's latest revision-request comment inline, so staff don't have to open the order to see what to fix) with an inline "Upload design proof" button per row, and a read-only "Waiting on the customer" list (status `proof_uploaded`) for visibility into what's currently in the customer's court. Upload calls the exact same `upload-url` + `publish-proof` endpoints `OrderDetailClient`'s own staff upload button already used — one server-side upload code path, two UI entry points into it.

**Nothing new in the database.** No new tables, columns, or API routes — this is entirely a new read/aggregate view over `portal_orders`, `portal_order_files`, and `portal_order_approvals`, and reuses the four existing action endpoints (`approve`, `request-revision`, `files/upload-url`, `publish-proof`) as-is. No SQL migration to run for this one.

Verified: `npx tsc --noEmit` clean on `apps/web`; `npx eslint` clean on all six new/changed files. One lint fix needed along the way — `react-hooks/set-state-in-effect` flagged calling a memoized `refresh()` directly inside `useEffect` in the staff tab; wrapped it in an inline async IIFE (matching `OrdersTab.tsx`'s own existing load-effect shape) to resolve it, with no behavior change.

## 23. LFG Connect Site Master: status filter replaced with real-signal facets, cards show courier + expected delivery (16 Sept 2026)

Task feedback (Mahin, verbatim): "this filter are becoming meaningless so remove some thing we are not updating frpom cards" followed by "the filetr of statusses should be like Active / inactive Site Survey Available / non available Creative received / not received printed / not printed shipped/not shipped delivered/ not delivered installed / not installed. Also on cards add if it shipped it should display how it is shipped i mean Bluedart / by hand / World first courier/ DTC etc. along with expected data of delivery. Then followed by trackign details if it is dsent through Bluedart."

**Investigated before writing anything.** The old filter was a single dropdown built straight off `site_status`'s 18-value enum (`LFG_STATUSES`) — a site's `site_status` is set by hand (a "Change Status" picker on Site 360, one restricted one-tap button, or a handful of auto-advance triggers), so a site whose real-world paperwork/shipment/installation had clearly moved on could sit on a stale status no one had bothered to update, making a status-only filter unreliable exactly as described. Meanwhile several of the facts the request actually cares about already exist as real, independently-updated signals elsewhere in the schema: a Site Survey document on file (`lfg_site_documents`, category `survey`), `lfg_sites.creative_received_at`, a shipment row with an AWB (`lfg_shipments`), and `lfg_installations.installation_status`.

**Replaced the dropdown with seven independent Yes/No facet toggles** (`workspaces/lfg/page.tsx`): Active/Inactive, Site Survey Available/Not, Creative Received/Not, Printed/Not, Shipped/Not, Delivered/Not, Installed/Not. Each backed by whichever signal is actually the real source of truth, matching what the card itself already shows rather than inventing a second, conflicting definition:
- **Active/Inactive** — `site_status` is/isn't `deactivated`/`deactivation_requested`. Archived sites (`archived_at is not null`) stay fully hidden from Site Master always, exactly as today — confirmed with Mahin before building (locked-in answer: "Archived stays hidden always") — this facet only ever splits the already-visible, non-archived sites.
- **Site Survey** — real signal: a `lfg_site_documents` row with `category = 'survey'` exists, the same thing that decides whether a card's own "Site Survey" button is enabled.
- **Creative / Printed** — reuse `lfgBenchmarkStatus()`'s existing `creative_received`/`in_production` checkpoints as-is (same function `LfgBenchmarkStrip` already renders on every card), rather than a second definition — "Printed" has no better real signal available (`lfg_production` is written but read nowhere in the UI, confirmed by search).
- **Shipped** — real signal: a shipment row with a non-null AWB exists, the same predicate the card's own AWB section already uses (not "site_status reached dispatched").
- **Delivered** — `site_status` rank ≥ `delivered`, OR the site's latest shipment's own `current_status` is `delivered` — the exact same either/or `trackingSummary()` already uses for the card's "Tracking" badge.
- **Installed** — real signal: `lfg_installations.installation_status = 'completed'`, the same field the card's own "Installation" badge reads.

Filtering itself is client-side (`displayRows`, a `useMemo` over the already-loaded `rows`) since three of the seven facets need child-table data no single `lfg_sites` query carries. A new effect fetches survey-doc existence / shipment AWB+status / completed-installation-status for every currently-loaded row's `id`, chunked into batches of 150 `.in()` ids (same reasoning as the existing `.range()` paging on the main site fetch), keyed on the row-id-set string rather than `rows` itself so an in-place status edit (e.g. a quick-action button) doesn't re-trigger the lookup. `siblingCounts`, the "Showing"/"Need Attention" stat pills, the table's "select all shown" checkbox, and both the List and Cards views all now read `displayRows` instead of the raw fetched `rows`, so every one of them agrees with what the facets actually narrowed down to. Every facet also round-trips through the URL (`?f_survey=yes` etc.), same as the other filters on this page, so Back navigation restores it.

**Site cards** (`LfgSiteCardGrid.tsx`): courier name was already being fetched (only ever used to decide whether to show the Blue Dart button) and is now shown as plain text ("Shipped via —"), alongside `expected_delivery_date` (fetched from `lfg_shipments` for the first time — it existed on the table but nothing read it before). Both show for any shipment on file, not just Blue Dart's — the Blue Dart-only section further down stays reserved for live tracking, which only that courier's integration supports, exactly as before (reused, not rebuilt: same `/api/lfg/shipments/[shipmentId]/track` route and `isBlueDartCourier()` regex). That route's final response now also re-selects `expected_delivery_date` (previously only `current_status`/`current_location`/`last_tracked_at`) so a fresh "Track via Blue Dart" call refreshes the shown date live, the same way it already refreshed the other three fields.

**Nothing new in the database.** No schema change, no migration — every field used (`lfg_site_documents.category`, `lfg_sites.creative_received_at`, `lfg_shipments.awb_number`/`current_status`/`expected_delivery_date`, `lfg_installations.installation_status`) already existed.

Verified: `npx tsc --noEmit` clean on `apps/web`; `npx eslint` clean on all three changed files (one fix needed along the way — `react-hooks/set-state-in-effect` on the new facet-signal effect's early-return branch, same pattern/fix as the pre-existing `siblingCounts` effect a few lines above it).

## 24. Same facet filters brought to the LFG partner home page (16 Sept 2026)

Task feedback (Mahin, verbatim): "Implement the same to customer partner site too." Section 23's seven-facet filter (Active/Survey/Creative/Printed/Shipped/Delivered/Installed) only went onto the staff Site Master (`workspaces/lfg/page.tsx`) — the LFG partner home page (`app/lfg/(app)/page.tsx`, the "Your Sites"/"All Sites" screen a genuine partner login like Apple actually lands on) had the exact same single `site_status` dropdown, unchanged.

**Extracted the facet logic into a shared module** (`@/lib/lfg-site-facets.ts`) instead of copy-pasting section 23's definitions into a second file: `FacetKey`/`FacetValue`/`EMPTY_FACETS`/`FACET_DEFS`/`SiteFacets`/`ShipmentSignal`/`computeSiteFacets`/`chunk`, all moved there and re-exported into the staff page (no behavior change there — confirmed via `tsc`/`eslint`), then imported into the partner page too. The point of doing this as a shared module rather than duplicating the block: the staff Site Master and the partner home page can now never end up with two different definitions of what "Shipped" or "Delivered" means — exactly the kind of drift this whole task started by trying to get rid of. (The three child-table queries themselves — survey docs / shipments / installations — stay duplicated, one copy per page's own effect; supabase-js's chainable query builder type doesn't narrow cleanly through an extra function boundary, so only the pure filtering logic is shared, not the query code itself.)

**Partner page changes**, otherwise identical in shape to section 23's staff-page changes: the status `<select>` is replaced by the same seven Yes/No toggle row, a new effect fetches survey/shipment/installation signals for the currently-loaded rows (chunked `.in()`, same 150-id batches), a `displayRows` memo narrows `rows` by whichever facets are on, and the stat cards / Table / `LfgSiteCardGrid` / "no sites match" empty state all switched from `rows` to `displayRows`. Facets round-trip through the URL (`?f_survey=yes` etc.) same as every other filter on this page already does. The "My Sites"/"All Sites" toggle, the Program/Format filters, and the RLS scoping (`.eq("partner_id", ...)`) are all untouched — facets apply on top of whichever of those is already selected, same relationship the staff page's facets have to its own format/program/store filters.

**`LfgSiteCardGrid.tsx` itself needed no changes for this section** — it's the exact same shared component both pages already render, so the courier-name/expected-delivery-date display and the Blue Dart tracking additions from section 23 apply here automatically; a partner logged in as e.g. Apple now sees the same "Shipped via <courier>" / expected delivery date text on their own cards that staff see on the Site Master.
...
## 26. LFG Connect: every Width/Height surface is mm-only now, no inches anywhere (16 Sept 2026)

Task feedback (Mahin, verbatim, with a screenshot of Site 360's "Edit Site Details" form showing Width=51990/Height=1245 under "Width (in)"/"Height (in)"): "admin site lfg sites sizes are in inches convert them into mm make all mm only no more inches."

**Found the actual bug, not just a label problem.** `lfg_sites.width`/`.height` have always been stored in inches by design (`lfg-units.ts`'s own header comment; the New Site form and Bulk Import both convert mm to inches before writing). Every READ-ONLY surface already converts inches→mm correctly for display (`formatMm`, real `× 25.4` math) — Site Master's table, Site Cards, the Status Sheet, and even the read-only summary card directly above this same edit form on Site 360. But the **Site 360 "Edit Site Details" form itself never did that conversion** — it read/wrote the raw inches number straight through, just under a "Width (in)"/"Height (in)" label. That's a real, silent data-corruption bug: anyone who read a site's real-world mm measurement off a spec sheet (or off the read-only mm card two inches above this exact form) and typed it into this field got it saved as if it were inches — e.g. typing "5190" (the site's real mm width) saves as 5190 *inches* (≈132 m), which is almost certainly what happened to the "ICONCEPT @ METROPOLIS MALL" record in the screenshot (raw stored width 51990 × 25.4 would be physically absurd; the sheet on the user's own second monitor showed this site's real size as 5190mm × 1245mm).

**Fixed by converting at the form boundary, not by migrating stored data.** `lfg_sites.width`/`.height` keep meaning exactly what they've always meant (inches) — every existing row's data is untouched, so this carries zero risk to the database. Only the three surfaces that ever WRITE these columns changed:
- **Site 360 Edit form** (`LfgSiteWorkspaceClient.tsx`): `siteToForm()` now converts the loaded value to mm (`inchesToMm`) before it ever reaches form state, the save handler converts back (`mmToInches`) right before the Supabase update, and the labels read "Width (mm)"/"Height (mm)". This is the actual bug fix — the form now does what its neighboring read-only card already did.
- **New Site form** (`workspaces/lfg/new/page.tsx`): the Inch/MM toggle is gone entirely (not just defaulted) — the Size fields are always mm, converted to inches only in the submit handler's `toInches()`.
- **Bulk Import** (`LfgSiteImportClient.tsx`): same removal — the "Width/Height/Bleed in Inch / in MM" toggle is gone, replaced with a plain "Width/Height/Bleed read as millimetres" label; every sheet's Width/Height/Bleed columns are read as mm and converted once on the way in.
- **LFG partner home page** (`app/lfg/(app)/page.tsx`): dropped the "Size (in)" combined column from the site table — the existing separate Width (mm)/Height (mm) columns already cover it, and it was the one remaining place a raw (correctly-labeled) inches figure was shown to anyone.

`lfg-units.ts`'s `formatSizeInches()` helper is now unused everywhere and was removed; `mmToInches`/`inchesToMm`/`formatMm`/`formatSizeMm` are unchanged and now the only unit-conversion helpers this file needs.

**What this does NOT fix**: any site already saved through the old buggy Edit form has a genuinely wrong `width`/`height` value sitting in the database — this change only stops it from happening to any more sites going forward. Gave Mahin a query to spot-check for likely-corrupted rows separately (not run as part of this change, since it's read-only and just for his own review): `select site_id, outlet_name, width, height from lfg_sites where width > 300 or height > 300;` — a genuine inches width/height for retail signage is essentially always well under 300; anything above that is almost certainly an mm figure that got saved as inches by the old form.

**Deliberately left alone**: `bleed` and `sqft` — neither has ever had a defined inches/mm convention in the New Site or Edit forms (both just `round2()` whatever's typed, no conversion), and that's a separate pre-existing quirk out of scope for this task, which was specifically about Width/Height. `measured_width`/`measured_height` on the Site Survey tab (a different table, `lfg_site_surveys`) were also left untouched — no evidence they follow the same inches convention, and the task's own wording ("lfg sites sizes") points at `lfg_sites.width`/`.height` specifically.

No SQL, no schema, no DB migration — every change is app-code-only, converting at each write/read boundary rather than touching what's already stored.

Verified: `npx tsc --noEmit` clean on `apps/web`; `npx eslint` clean on all five changed files (`lib/lfg-units.ts`, `components/workspaces/LfgSiteWorkspaceClient.tsx`, `app/workspaces/lfg/new/page.tsx`, `components/workspaces/LfgSiteImportClient.tsx`, `app/lfg/(app)/page.tsx`).

## 27. LFG Connect: new "Edit Sizes" tab — filterable, Excel-style bulk row editor (16 Sept 2026)

Task feedback (Mahin, verbatim): "give me a sparate tab with all sites by all filters to edit on row itest to correct the sizes at once. like excel edit. I just need format of store, SFO iD, store name , City, material, Width heght, , bleed Qty Partner so that i can edit the sizes at once."

New page: `apps/web/src/app/workspaces/lfg/sizes/page.tsx`, new "Edit Sizes" tab in `LfgConnectNavBar.tsx` (Ruler icon). Fetches every non-archived site once (same "fetch all, filter client-side" shape as Site Master), shows one row per site with Format/SFO ID/Store Name/City/Partner as read-only context and Material/Width/Height/Bleed/Qty as inline-editable inputs — an edit just changes local `drafts` state (mm units for size fields, converted to inches only at save via `mmToInches()`), a dirty row gets a small revert icon, and one "Save changes (N)" button batches every dirty row's `.update().eq("id", id)` call. Filters: search + Program + Format + City + Partner selects, matching Status Sheet's convention. Validates before saving (Width/Height/Bleed must be numeric if non-blank; Qty must be a whole number ≥ 1, since `number_of_sites` is `not null default 1`) and blocks the whole save with a toast naming the first bad row rather than partially saving.

**Also fixed a live version of the same inches/mm corruption bug this session already fixed elsewhere (section 26)**, found while researching this page: Status Sheet's existing `SiteFieldsEditControl` popover (the other place Width/Height can already be edited) labels its fields "Width (mm)"/"Height (mm)" but was reading/writing the raw inches column with zero conversion — identical bug class to the Site 360 Edit form fixed in section 26, just never caught there. Fixed the same way: `inchesToMm()` when the popover opens/loads, `mmToInches()` right before the Supabase update. Not something Mahin asked about directly this time, but it's the exact "make all mm only, no more inches" bug the previous task was about, still live in one more place — left unfixed it would have kept corrupting rows going forward, including ones just corrected via the new Edit Sizes tab.
...
## 29. LFG Connect: new "Shipping Labels" feature — A4-portrait per-site labels, address + Site Photo & Measurement (19 Sept 2026)

Task feedback (Mahin, verbatim, with a Site Survey Report reference screenshot of an Apple "SITE PHOTO & MEASUREMENT" page): "Now do you have the stopre address and phone number for all sotes an stores? Lets generate address lable in A4 portrait size half of the page to Address and bottom half of the page measurement page like attached. as shipping lable." Scope confirmed via three follow-up questions, all answered with the recommended option: (1) a site with no completed Site Survey Report gets a text-only Measurements & Material table instead of a fabricated/missing photo; (2) build as a real, reusable LFG Connect feature, not a one-off script; (3) cover every active (non-archived) site.

New self-contained PDF builder: `apps/web/src/lib/shippingLabel/labelPdf.ts`, exporting `buildShippingLabelsPdf()` — one A4 **portrait** page per site (pdf-lib, client-side, no server round-trip, same approach as Site Survey Report's own `pdfBuild.ts`). Top half is a "SHIP TO" address block (outlet name, SFO ID, Program, address/city/state, ASM name/mobile/email, escalation email — all straight off `lfg_sites`). Bottom half is a "SITE PHOTO & MEASUREMENT" block: when the caller supplies a `measurement` (i.e. the site has a usable completed survey), it draws the real photo with its yellow installation-area annotation plus a "Facade" dimension-arrow diagram, then a Measurements & Material table sourced from the survey; when it doesn't, it draws a plain text-only Measurements & Material table built from `lfg_sites`' own Width/Height/Bleed/Material/Qty instead (no photo, no diagram — matches the confirmed fallback). Deliberately built as a **new, self-contained file rather than reusing `pdfBuild.ts`'s internals**: that file exports almost nothing usable (only types), is hardcoded to A4 landscape vs this label's required portrait, and carries an SF Pro/Apple-brand typography system this internal label doesn't need. Only genuinely-exported pieces (`normalizeAnnotation` and the annotation/measurement types from `siteSurveyReport/types.ts`) are reused; the photo-clipping, annotation-polygon, and Facade-diagram drawing logic is a fresh, simpler port of that file's own `drawPhotoBox`/`drawAnnotation`/`drawFacadeDiagram`, at label scale.

New page: `apps/web/src/app/workspaces/lfg/shipping-labels/page.tsx`, new "Shipping Labels" tab in `LfgConnectNavBar.tsx` (Truck icon). Lists every non-archived site (same fetch-all shape as Edit Sizes/Site Master), with Program/City/"has a completed survey" filters, a bulk-select checkbox column (same `Set<string>` + "select all shown" pattern as Site Master's own bulk actions), a per-row "Label" button, and a bulk "Generate Labels (N)" action. "Usable survey" means a `site_survey_reports` row for that site with status `ready` or `generated` (draft/extracting/review_required don't count as completed) — computed once from a single summary query, so the list itself stays cheap; the actual measurement + photo (fetched via the same signed-URL API route pattern as `SiteSurveyReportEditorClient`'s `fetchPhotoInputs()`) is only pulled per site at the moment a label is actually generated. A photo fetch failure is swallowed rather than blocking the label — the PDF's own `drawPhotoBox` placeholder covers that case.

Verified: `npx tsc --noEmit` clean; `npx eslint` clean on all three new/changed files (`lib/shippingLabel/labelPdf.ts`, `app/workspaces/lfg/shipping-labels/page.tsx`, `components/workspaces/LfgConnectNavBar.tsx`).

## 30. Shipping Labels: crash on the "CUT HERE" divider's scissors character (19 Sept 2026)

User-reported crash (screenshot): `WinAnsi cannot encode "✂" (0x2702)` when generating a label. Cause: the cut-line divider's `"✂ CUT HERE"` text used a scissors glyph, and pdf-lib's non-embedded standard Helvetica font is WinAnsi-encoded — it can't render that Unicode character at all. Fixed in `labelPdf.ts` by replacing it with plain hyphens (`"- - CUT HERE - -"`). Checked the rest of the file for any other non-ASCII character (`grep -nP '[^\x00-\x7F]'`) — the remaining two (em dash, multiplication sign) are both valid WinAnsi/CP1252 characters, safe to keep.

## 31. LFG Connect: six fixes/features from a single bug-report batch (19-22 Sept 2026)

Task feedback (Mahin, verbatim): "LFG Connect: Filters Printed - not printed, installed - not installed filters not working check. Process steps : from active, reactive not available to mark. Process steps: once wrongly marked that creative received can not go back on 'status sheet' and there is no option for caressive received implement. Once an out lets removed from from a program and can not call back . There is no option. Fix it. For status updated. Person logs in tot portal. From MMDI side there is no option given to update INS sites. bBut MMDI printed them to creative received them too. Like only installation is done by INS east all done by MMDI. So fix that login prevelege. Introduce principle Apple Partners : Like Aptronix, iMagine iAstra some times that stores taken over between Apple partners and the name is changing but rest all detail remains same. So need provision to switch and takeover action." Six distinct items, researched in full before any code was written; four scope questions resolved via follow-up (below). The fifth item (INS login privilege) is **not** included in this batch — pending a fuller description from Mahin, see its own note at the end.

**1. "Printed"/"Installed" facet filters weren't reliable.** Both were previously derived indirectly: "Installed" came from `lfg_installations.installation_status`, but "Printed" was inferred from `lfgBenchmarkStatus()`'s rank threshold on `site_status` rather than any real production signal — so a site could show as printed/not-printed inconsistently with what Production actually recorded. `computeSiteFacets()` (`lib/lfg-site-facets.ts`) now takes `printed` as a real parameter, sourced the same way `installed` already was: a `lfg_production.status = 'completed'` lookup, queried alongside the existing `lfg_installations` query on both Site Master pages (`workspaces/lfg/page.tsx` and `lfg/(app)/page.tsx`).

**2. Production/Installation completion now advances `site_status` forward (never backward).** So the underlying signal and the site's own status stop disagreeing going forward: marking Production or Installation "Completed" on a Site 360 (`LfgSiteWorkspaceClient.tsx`'s `ProductionTab`/`InstallationTab`) now also calls `lfg_change_site_status` to advance to `in_production`/`installation_completed`, but **only if** the site hasn't already progressed further through some other path (`LFG_STATUSES.indexOf(current) < LFG_STATUSES.indexOf(target)`) — so this never regresses an already-Shipped site back down. The LFG Connect partner portal's "Mark Printed"/"Mark Installed" quick-status buttons (`LfgPartnerQuickStatusButtons.tsx`) now also best-effort-upsert the matching `lfg_production`/`lfg_installations` row when their status RPC fires, so the facet signal stays in sync from that side too — made non-blocking/best-effort deliberately, since `lfg_production`'s write RLS has no partner clause at all (staff-only) and this button is also reachable by the one full-lifecycle-partner login; a failure there must never break the button's actual (working) status-change contract.

**3. Reactivate a deactivated/on-hold site.** Previously a site marked `deactivated`/`deactivation_requested`/`on_hold` had no path back to `active` short of the generic status-change dialog buried in Site 360. Added a one-click **Reactivate** button directly on the Status Sheet (`workspaces/lfg/status-sheet/page.tsx`), shown only for sites in one of those three statuses, calling `lfg_change_site_status` straight to `active`.

**4. Undo a wrongly-marked "Creative Received".** Same page: added a Received/Awaiting toggle control (mirrors the existing Site 360 Survey tab's own mark/undo pattern) that writes/clears `creative_received_at`+`creative_received_by` directly — so a wrong mark no longer needs a database fix to reverse.

**5. A removed-from-program site couldn't be reassigned back.** The 11 Sept archive migration sweeps up any site with `program_id is null`, and Restore (`workspaces/lfg/archive/page.tsx`) only ever wrote `archived_at = null` — leaving `program_id` null, so the site landed back in the live list with no program and, worse, was liable to be swept right back into the archive by the same migration logic. `handleRestore` now also accepts a program to assign; when a site's `program_id` was already set it restores in one click as before, but when it's null the Restore control opens a small popover to pick a program (or leave it unassigned) before restoring. Confirmed via AskUserQuestion, also added a plain **Active/Inactive toggle** on each Program card (`workspaces/lfg/programs/page.tsx`) — previously `lfg_programs.active` was always `true` and never written anywhere in the app — so a program can be wound down without needing every site under it individually archived.

**6. "Takeover" — reseller partner handover between Apple partners (Aptronix, iMagine, iAstra, ...).** New `hq_partner` concept, confirmed via AskUserQuestion as a dedicated action rather than a plain editable field. Added `lfg_stores.hq_partner` (`supabase-lfg-stores-hq-partner-migration.sql`, backfilled from `lfg_sites.hq_partner`, validated with `pglast`) so it behaves as a genuine store-level field — same treatment `outlet_name`/`format`/ASM fields/etc. already get. `LfgSiteWorkspaceClient.tsx`'s `SiteInfoCard` (staff Site 360 only — the LFG Connect partner-facing Site 360 doesn't render this card) shows the current Reseller Partner and a new **Takeover** button next to Edit, opening a purpose-built `TakeoverDialog`: shows the current partner, a text field (with autocomplete over existing values) for the new one, and a live preview of every other display at the same store the change will also apply to. Confirming writes `hq_partner` to `lfg_stores` + every sibling `lfg_sites` row sharing `store_id` (falling back to just the one row for a site with no store yet) — same dual-write shape as `SiteInfoCard.handleSave()`'s own store-field propagation, so every affected site's own Activity Log records the change (`hq_partner` added to that page's `KEY_LABEL` map as "Reseller Partner"). `workspaces/lfg/sites/[siteId]/page.tsx` already selects `*` so no change needed there; `lfg/(app)/sites/[siteId]/page.tsx`'s explicit column list was extended to include `hq_partner`.

**Not in this batch: INS login privilege.** Mahin's report ("From MMDI side there is no option given to update INS sites... Like only installation is done by INS east all done by MMDI. So fix that login prevelege") was initially read as an account-provisioning gap, but when asked to confirm that diagnosis via AskUserQuestion he chose "It's something else — let me describe it more." No code has been written for this item; needs his fuller description before proceeding.

Verified: `npx tsc --noEmit` clean; `npx eslint` clean across every changed file (`lib/lfg-site-facets.ts`, `workspaces/lfg/page.tsx`, `lfg/(app)/page.tsx`, `workspaces/lfg/status-sheet/page.tsx`, `workspaces/lfg/archive/page.tsx`, `workspaces/lfg/programs/page.tsx`, `workspaces/lfg/activity/page.tsx`, `lfg/(app)/sites/[siteId]/page.tsx`, `components/workspaces/LfgSiteWorkspaceClient.tsx`, `components/lfg/LfgPartnerSiteClient.tsx`, `components/lfg/LfgPartnerQuickStatusButtons.tsx`).

## 32. Item 1 follow-up: Site Cards' own checklist still disagreed with the Printed/Installed filter (19-22 Sept 2026)

Mahin, after #31 shipped: "Installed not installed filters not workign still" -- with a screenshot of the "Not installed" filter selected, showing cards whose own on-card checklist read "✓ Printed ✓ Shipped ✓ Delivered ✓ Installed" (all crossed) anyway. The filter itself was actually working correctly (#31 already moved it onto the real `lfg_production`/`lfg_installations` signal) -- the bug was that the card's own **Printed/Installed checkmarks never got the same fix**. `LfgBenchmarkStrip` (the six-checkpoint checklist shown on both Site Cards and the Status Sheet) has always been purely `site_status`-rank-derived, a deliberate cheap approximation documented in its own header comment -- so a site whose status had advanced far enough in the lifecycle showed "Printed"/"Installed" as crossed regardless of whether production/installation was ever actually marked complete, which is exactly what could disagree with a filter now driven by the real signal.

Fix: `LfgBenchmarkStrip` takes two new optional props, `printed`/`installed`, that override just those two checkpoints' crossed state when passed (Status Sheet, which doesn't fetch the real signal, is untouched and keeps the old rank-based behavior for both). `LfgSiteCardGrid` (and its inner `SiteCard`) now accept `printedIds`/`installedIds` Sets and pass `printed`/`installed` straight through to the strip. Both Site Master pages (`workspaces/lfg/page.tsx` staff, `lfg/(app)/page.tsx` partner) already compute these exact Sets for their own filters (#31) -- now passed into `<LfgSiteCardGrid>` too, so the same real data drives both what's filtered and what the card displays, instead of two different definitions of "Installed" disagreeing on-screen.

Verified: `npx tsc --noEmit` clean; `npx eslint` clean (`components/workspaces/LfgBenchmarkStrip.tsx`, `components/workspaces/LfgSiteCardGrid.tsx`, `app/workspaces/lfg/page.tsx`, `app/lfg/(app)/page.tsx`).

## 33. Item 1, real root cause: Printed/Installed filter was correct, the underlying DATA was the gap (22 Sept 2026)

Mahin, after #32 shipped: "filter: installed not working also not installed showing even installed ones." #31/#32's code was correct -- the bug is data, not logic. The Printed/Installed filters (and now the card checklist) read `lfg_production.status`/`lfg_installations.installation_status`, which only get written to `'completed'` by going through the app's Production/Installation tabs or the partner "Mark Printed"/"Mark Installed" quick-actions -- and #31 item 2 only made those write BOTH signals together **going forward**. Any site whose status was advanced the old way (the generic Change Status dropdown, a direct DB edit, or an import) before that sync existed -- almost certainly most of the existing 325 sites -- never got an `lfg_production`/`lfg_installations` row created or completed at all, even though its own `site_status` plainly shows it shipped, was delivered, or is fully installed. So the real-signal filter was reading exactly what's on file and correctly calling those sites "not printed"/"not installed" -- it's the data on file that was incomplete, not the filter logic.

New one-time migration: `supabase-lfg-production-installations-backfill.sql`. For every non-archived site, backfills `lfg_production.status = 'completed'` for any site whose `site_status` rank (LFG_STATUSES' own fixed order) is at or past `in_production`, and `lfg_installations.installation_status = 'completed'` for any site at or past `installation_completed` -- same rank-based simplification the six-checkpoint benchmark checklist already documents and relies on for its own end-states (Active/On Hold/Deactivated/etc. count as past every earlier checkpoint). Idempotent (`on conflict (site_id) do update ... where <col> <> 'completed'`) -- only ever moves a row TO completed, never away, so it's safe to run again later and never overrides a genuinely early-stage site. Validated with `pglast`.

This is a one-time catch-up for existing sites; #31 item 2's write-path sync keeps it from drifting again going forward for anything changed through the app from now on.
