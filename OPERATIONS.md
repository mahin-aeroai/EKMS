...
| `BLUEDART_CONSUMER_KEY` / `BLUEDART_CONSUMER_SECRET` | Blue Dart JWT auth (`GET /v2/login`), shared by all three Blue Dart features below (`src/lib/blueDart.ts`) | Vercel env vars only | Created by Srinivas himself on developer.dhl.com: Register → Create App → select **Blue Dart API Suite (DHL eCommerce India, Blue Dart)** (the portal bundles Auth + Tracking + Finder + Transit Time into one product, unlike the older per-API breakdown in Blue Dart's manual) → the App's page shows "API Key" (= Consumer Key) and "API Secret" (= Consumer Secret) under Show Key. Not the same as the credentials Blue Dart emailed directly (those two below) |
