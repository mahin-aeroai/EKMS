...
- Every "AI insight" card outside the AI Copilot chat is still static/
  lightly-templated copy, not generated by a model — that's unchanged. The
  AI Copilot chat itself IS now real (see item 20 in session history):
  actual Claude API calls with tool-use grounding in live Supabase data.
  Requires `ANTHROPIC_API_KEY` set in Vercel — without it, the route
  returns a clear "not configured" message rather than crashing, and the
  UI shows a toast explaining why. The 4 flagship workspaces' "Ask about
  this record" boxes are still demos — deliberately out of scope (see the
  AI Copilot session history entry for why).
