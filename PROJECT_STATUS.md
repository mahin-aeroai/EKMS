...
- The 4 flagship workspaces' Insights/Timeline/Documents/Relationships tabs
  ...
  back them with yet.
- Machine and Raw Material workspaces are pointed at *whichever row
  happens to be most recently created* in their table (no known real demo
  record was picked, unlike Customer's `C03739`) — worth pointing them at
  specific real records once you know which machine/material should be the
  go-to demo one. Project workspace still has zero rows to point at — no
  project data has been uploaded yet.
