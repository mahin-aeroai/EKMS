...
-- Digital, Croma_2.0 [superseding the older Croma sheet], WC). 186
-- of them come through with a NULL hq_partner even though the
-- spreadsheet has SOME value in that cell -- see STEP 2's own comment
-- below for why.
--
...
--
-- Deliberately has NO staging table at all (an earlier version of this
-- script used one -- first a TEMP table, then, when that still errored,
-- a regular table -- and Srinivas hit "relation does not exist" both
-- times). Supabase's SQL Editor can run a pasted multi-statement script
-- across more than one pooled backend connection, so anything one
-- statement creates is not guaranteed visible to the next. Every
-- statement below is fully self-contained instead: the same 385-row
-- `import` list is repeated inline (as a WITH ... AS (VALUES ...) CTE)
-- in each statement that needs it, so nothing has to survive between
-- statements. Slightly more text in this file; zero chance of this
-- particular error.
--
-- Run the whole file in one go, top to bottom -- each statement is
-- independent, but the diagnostics at the end make the most sense read
-- after the two updates above them have run.
-- ============================================================

-- ============================================================
-- STEP 1 -- fill hq_partner, matched by sfo_id. Only overwrites a NULL
...
-- script is safe. 186 of the 385 import rows carry a NULL
-- hq_partner on purpose: their actual spreadsheet cell (in the Vijay
-- Sales, Reliance Digital, and Croma/Croma_2.0 sheets) held junk like
-- "Multi AAR"/"White"/"W/W+"/"Apple Shop 3.0" instead of a real company
-- name, so there's nothing genuine to import for those rows.
-- ============================================================

with import(sfo_id, store_name, hq_partner, installation_team, source_sheet) as (
  values
...
-- ============================================================
-- STEP 2 -- fill partner_id (the Execution Partner), matched by sfo_id,
...
-- ============================================================
-- STEP 3 -- diagnostics. Three separate lists, showing what's still
-- missing after the two updates above:
-- ============================================================
...
--     Partner cell for that row wasn't a real company name (see STEP
--     1's comment) -- hq_partner is still null for these; the real
--     value has to come from Srinivas directly, this script can't
--     guess it.
with import(sfo_id, store_name, hq_partner, installation_team, source_sheet) as (
  values
    ('1300789', 'IPLANET @ BANASHANKARI', 'CONSOLIDATED PRIVATE LIMITED', 'I&S', 'APR- APP'),
...
join import i on i.sfo_id = s.sfo_id
