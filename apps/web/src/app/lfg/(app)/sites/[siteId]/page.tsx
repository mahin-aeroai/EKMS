...
export default async function LfgPartnerSiteDetailPage({ params }: { params: Promise<{ siteId: string }> }) {
  ...
  const { data: site, error: siteError } = await supabase
    .from("lfg_sites")
    .select(
      "id, site_id, outlet_name, program, sfo_id, city, store_address, material, number_of_sites, width, height, sqft, asm_name, asm_mobile, asm_email, escalation_email, remarks, site_status, creative_received_at, site_reference_picture_path, partner_id, lfg_partners(id, name)"
    )
    .eq("id", siteId)
    .maybeSingle();
