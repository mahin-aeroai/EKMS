...
import type { PurchaseOrderRow } from "@mmdi/shared/rows";
...
export default function ProcurementPage() {
  ...
  const openPOs = columns?.reduce((sum, c) => sum + c.cards.length, 0) ?? null;
  ...
  return (
    <div>
      ...
      <div className="my-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard label="Open POs" value={openPOs === null ? "—" : String(openPOs)} />
        <StatCard label="Avg Cycle Time" value="—" />
        <StatCard label="Spend MTD" value="—" />
      </div>
